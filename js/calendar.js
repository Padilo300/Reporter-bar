$( document ).ready(function() {
    ...
    function getSchedule_user_1() {
        ...
        function(data) {
            $('#table-schedule-reporter  .fact-row-1 .dayGrid').remove()     ; //удалить старую сетку рабочих дней
    ...
    function getSchedule_user_2() {
        ...
        function(data) {
            //получает ответ JSON массив true для рабочих ней и false для выходных
            $('#table-schedule-reporter  .fact-row-2 .dayGrid').remove()     ; //удалить старую сетку рабочих дней
            data = $.parseJSON(data);
            for (var i = 0, d = 1, a = 1; i < dInMonth(Month,Year); i++) {
                if (data[i] === 'true') {   
    ...
    function getSchedule_user_3() {
        ...
        function(data) {
            $('#table-schedule-reporter  .fact-row-3 .dayGrid').remove()     ; //удалить старую сетку рабочих дней
            //получает ответ JSON массив true для рабочих ней и false для выходных
            data = $.parseJSON(data);
            for (var i = 0, d = 1, a = 1; i < dInMonth(Month,Year); i++) {
                if (data[i] === 'true') {   
        ...
        function(data) {
            $('#table-schedule-reporter  .fact-row-4 .dayGrid').remove()     ; //удалить старую сетку рабочих дней
            //получает ответ JSON массив true для рабочих ней и false для выходных
            data = $.parseJSON(data);
            for (var i = 0, d = 1, a = 1; i < dInMonth(Month,Year); i++) {
                if (data[i] === 'true') {   
    ...
    function countMoney(dayWork){ return Math.floor($('#salary').text()/dayWork)}   ; //узнаем сколько стоит одна рабочая смена
    function prinGrid() {
    ...
    function calculateMoney(){
        ...
        $('.fact-row-count .dayGrid').click(function(){            
            if (count<7000) {
                $('#money i').html('<i class="fa fa-arrow-down" aria-hidden="true"></i>');
                $('#money , #money-value').addClass('lostMoney');
            }
            if (count>7000) {
                $('#money i').html('<i class="fa fa-arrow-up" aria-hidden="true"></i>');
                $('#money , #money-value').addClass('addMoney');
            }
            if (workingDayR1Fact > workingDayR1) {
                $('#money i').html('<i class="fa fa-arrow-up" aria-hidden="true"></i>');
                $('#money , #money-value').addClass('addMoney');
    ...
    $('#table-schedule-reporter .fact').on('click','.dayGrid',function(){
        if ($('#table-schedule-reporter .fact').hasClass('true-user')) {
    ...
    $('#Table-Rep-next_Month').click(function(e){
        ...
        // setTimeout(calculateMoney(), 10000);
        setInterval(function() {
            calculateMoney();
        }, 500);
        
        var workingDayR1        =    $('#table-schedule-reporter .table-schedule__row .row-1  .black').length    ; // Кол-во выходов по графику
        var workingDayR1Fact    =    $('#table-schedule-reporter .ftable-schedule__row .fact-row-1  .black').length    ; // Кол-во выходов по факту   
        $('#table-schedule-reporter .fact-row-1').append(
                '<td class="table-schedule_border1 dayGrid"  data-user="1">' + workingDayR1Fact + '</td>'
        );        
	...
	for (var i = 0; i < nowMonth; ++i) {
        $('#Table-Rep-next_Month').trigger('click');
    }    
